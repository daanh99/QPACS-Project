...
GCLOUD_KEY_PATH = 'quantitative-293316-7974c0f3a5b7.json'  # The path to the Service Account Key (a JSON file)
...
class MasterNode(Node):
    def start_type(self):
        stdin, stdout, stderr = self.ssh.exec_command(f'echo "SPARK_MASTER_HOST=\'{self.privip}\'" >> /home/{home_user}/bd/spark/conf/spark-env.sh')
        if (len(stderr.read()) > 0):
            print(stdout.read())
            print(stderr.read())

        stdin, stdout, stderr = self.ssh.exec_command(f'/home/{home_user}/bd/spark/sbin/start-master.sh')
        if len(stderr.read()) > 0:
            print(stdout.read())
            print(stderr.read())
    ...
    def submit(self, options: JobOptions):
        ...
        stdin, stdout, stderr = self.ssh.exec_command(f"/home/{home_user}/bd/spark/bin/spark-submit --master {self.privip} --driver-cores 1 "
...
class SlaveNode(Node):
    def start_type(self):
        stdin, stdout, stderr = self.ssh.exec_command(f'/home/{home_user}/bd/spark/sbin/start-slave.sh spark://{self.master.privip}:7077')
        if len(stderr.read()) > 0:
            print(stdout.read())
            print(stderr.read())

    def stop_type(self):
        stdin, stdout, stderr = self.ssh.exec_command(f'/home/{home_user}/bd/spark/sbin/stop-slave.sh')
        if len(stderr.read()) > 0:
            print(stdout.read())
            print(stderr.read())
...
for idx, row in experiments.iterrows():
    ...
    stdin, stdout, stderr = master.ssh.exec_command(f"/home/{home_user}/bd/spark/bin/spark-submit --master spark://{master.privip}:7077 --driver-cores 1 "
                                                    f"--driver-memory 1G --total-executor-cores {int(row.num_nodes)} --executor-cores 1 --executor-memory 1024M "
                                                    f"--py-files /home/{home_user}/bd/spark/lib/bigdl-0.11.0-python-api,/home/{home_user}/bd/codes/bi-rnn.py "
                                                    f"--properties-file /home/{home_user}/bd/spark/conf/spark-bigdl.conf "
                                                    f"--jars /home/{home_user}/bd/spark/lib/bigdl-SPARK_2.3-0.11.0-jar-with-dependencies.jar "
                                                    f"--conf spark.driver.extraClassPath=/home/{home_user}/bd/spark/lib/bigdl-SPARK_2.3-0.11.0-jar-with-dependencies.jar "
                                                    f"--conf spark.executer.extraClassPath=bigdl-SPARK_2.3-0.11.0-jar-with-dependencies.jar /home/{home_user}/bd/codes/bi-rnn.py "
